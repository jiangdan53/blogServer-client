<template>
  <div class="app-container">
    ...
    <div style="margin-top: 15px">
      ...
      >
         <el-select  v-model="blogId" slot="prepend" placeholder="请选择总类">
          <el-option v-for="item in blogClass" :label="item.blogName" :value="item.id"></el-option>
        </el-select>
    ...
    <el-table :data="data" style="width: 100%" border>
      <el-table-column prop="date" label="序号" width="60" align="center">
        <template slot-scope="scope">{{ scope.$index + 1 }}</template>
           ...
           <el-table-column prop="title" label="博客总类类别" align="center">
        <template slot-scope="scope">{{ scope.row.blogClass.blogName }}</template>
      </el-table-column>

      <el-table-column prop="title" label="博客分类类别" align="center">
    ...
    <el-dialog title="编辑文章分类" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="总类">
            <el-select  v-model="form.blogId" placeholder="请选择总类">
          <el-option v-for="item in blogClass" :label="item.blogName" :value="item.id"></el-option>
        </el-select>
        </el-form-item>
      </el-form>
  ...
  methods: {
    fetchData() {
      ...
      getBlogClassAll().then(res=>{
        this.blogClass = res.data
      })
    },
    // 添加文章分类
    addBlogTypeHandle() {
      if (this.input) {
        addBlogType({ name: this.input, order: this.select,blogId:this.blogId }).then(() => {
          this.fetchData();
    ...
    deleteBlogTypeHandle({ id }) {
      this.$confirm(
        ...
        .then(() => {
          delBlogType(id).then(({data}) => {
            this.fetchData();
            this.$message.success(`删除成功 受影响的文章数量为${data.affectedSize} 条`);
          });
        })
        .catch(() => {
          this.$message({
