<template>
  <div class="app-container">
    <!-- 数据表格 -->
    <el-table :data="data" style="width: 100%"  border :row-style="{height:'80px',overflow:'hidden'}" >
      <el-table-column prop="date" label="序号" width="60" align="center">
        <template slot-scope="scope">{{ scope.$index + (currentPage - 1) * eachPage + 1 }}</template>
      </el-table-column>

      <el-table-column prop="title" label="文章名称" width="150" align="center" min-width="150px">
      ...
      <el-table-column prop="title" label="文章描述" width="150" align="center" min-width="150px" >
          ...
          >
           ...
           {{scope.row.description}}
            ...
            <a
              ...
              @click.prevent="goToTitleHandle(scope.row)"
              slot="reference"
              >{{ scope.row.description.length > 16 ?  scope.row.description.substring(0,16)+"..."  : scope.row.description}}</a
            >
          </el-popover>
        </template>
      </el-table-column>

      <el-table-column prop="title" label="浏览数" width="150" align="center" min-width="150px"> 
        <template slot-scope="scope">{{ scope.row.scanNumber }}</template>
      </el-table-column>

      <el-table-column prop="title" label="评论量" width="150" align="center" min-width="150px">
        <template slot-scope="scope">{{ scope.row.commentNumber }}</template>
      </el-table-column>

      <el-table-column prop="title" label="所属总类" width="150" align="center" min-width="150px">
        <template slot-scope="scope">{{ 
          scope.row.category.blogClass === null ? '未分类' : scope.row.category.blogClass.blogName
        }}</template>
      </el-table-column>

      <el-table-column prop="title" label="所属分类" width="150" align="center" min-width="150px">
        <template slot-scope="scope">{{ 
      ...
      <el-table-column prop="title" label="创建日期" width="230" align="center" min-width="150px">
